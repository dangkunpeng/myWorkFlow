const fun = {
    layerArea:['80%', '80%'],
    confirmMsg:"该操作不可撤销,继续吗?",
    confirmBtns: {btn: ['确认', '返回']},
    formatField: function (field, fieldMap) {
        $("[data-field='" + field + "']").children().each(function () {
            const txt = fieldMap[$(this).text()];
            if (txt) {
                $(this).text(txt);
            }
        });
    },
    listToMap: function (dataList, key, val) {
        var dataMap = {};
        if (dataList && dataList.length > 0) {
            dataList.forEach(function (item) {
                dataMap[item[key]] = item[val];
            });
        }
        return dataMap;
    },
...
$.fn.extend({
    formData: function () {
        ...
        var keyValPairs = $(this).serializeArray();
        // 遍历key val
        $.each(keyValPairs, function () {
...
function ajaxPost(url, param, successCallback, errorCallback) {
    $ajax(url, "post", param, successCallback, errorCallback);
}
